    ...
    fun loadWifiNetworks() {
        ...
        wifiNetworkJob = viewModelScope.launch { // ViewModel 생명주기에 묶어 Wi-Fi 목록 로딩
            runCatching { wifiNetworkRepository.loadNetworks() }
                .onSuccess { networks ->
                    val connectedNetwork = networks.firstOrNull { it.isConnected } ?: networks.firstOrNull() // 현재 연결된 Wi-Fi를 우선 사용
                    val nextSsid = connectedNetwork?.ssid.orEmpty() // Android가 알려준 현재 SSID
                    uiState = uiState.copy(
                        wifiNetworks = networks, // 읽어온 Wi-Fi 후보 목록 저장
                        ssid = nextSsid, // 1차 구현에서는 사용자가 수정하지 않고 현재 연결된 SSID를 사용
                        safePayloadPreview = buildSafePayloadPreview(nextSsid, uiState.password), // 자동 선택된 SSID로 preview 갱신
                        ...
                        wifiNetworkMessage = if (networks.isEmpty()) {
                            ...
                            "현재 Android 기기가 연결 중인 Wi-Fi를 사용합니다. 비밀번호는 직접 입력해야 합니다."
